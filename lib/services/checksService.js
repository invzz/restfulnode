const {
  checkObjectAnyField, checkObjectAllFields, stringExactLenghtValidator,
  ...
  if (id) {
    ...
    dataPipe.read('checks', id, (checkErr, checkData) => {
      if (!checkErr && checkData) {
        ...
        authService.verifyAndAction(data, checkData.phone,
          () => {
            dataPipe.delete('checks', id, (delErr) => {
              if (!delErr) {
                dataPipe.read('users', checkData.phone, (usrErr, userData) => {
                  if (!usrErr && userData) {
                    let checkList = typeof (userData.checks) === 'object' && userData.checks instanceof Array ? userData.checks : [];
                    checkList = checkList.filter((item) => item !== id);
                    const updated = userData;
                    updated.checks = checkList;
                    dataPipe.update('users', checkData.phone, updated, (err, upUser) => {
                      if (!err) {
                        callback(200, upUser);
                      } else {
                        callback(500, err);
                    ...
                    callback(500, { Error: 'Could not find user who created the check. Could not update check list' });
                  }
                });
              } else {
