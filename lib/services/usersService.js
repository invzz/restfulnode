const dataPipe = require('../datapipes/fileDatapipe');
  ...
  if (phone) {
    authService.verifyToken(data, phone, (isAuthenticated) => {
      if (isAuthenticated) {
        dataPipe.read('users', phone, (err, storedData) => {
          if (!err && storedData) {
            dataPipe.delete('users', phone, (deleteError) => {
              if (!deleteError) return callback(200, { Deleted: storedData.phone });
              return callback(500, { Error: 'Could not delete the speciefied user' });
            });
          } else {
