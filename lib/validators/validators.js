...
const validateCheckFields = (check, validate) => {
  ...
  checkObj.protocol = ['https', 'http'].includes(check.protocol) ? check.protocol : false;

  if (typeof (check.url) === 'string') {
    const url = check.url.trim();
    checkObj.url = url.length > 0 ? url : false;
  } else {
    checkObj.url = false;
  }

  if (typeof (check.method) === 'string') {
    checkObj.method = ['post', 'put', 'get', 'delete'].includes(check.method.toLocaleLowerCase())
      ? check.method.toLocaleLowerCase()
      : false;
  } else {
    checkObj.method = false;
  }
  checkObj.successCodes = typeof (check.successCodes) === 'object'
    && check.successCodes instanceof Array
    && check.successCodes.length > 0 ? check.successCodes : false;

  checkObj.timeoutSeconds = typeof (check.timeoutSeconds) === 'number'
    && check.timeoutSeconds % 1 === 0
    && check.timeoutSeconds >= 1
    && check.timeoutSeconds <= 5 ? check.timeoutSeconds : false;

  checkObj.isValid = !!validate(checkObj);
  return checkObj;
};
...
validators.checkObjectAllFields = (check, callback) => {
...
validators.checkObjectAnyField = (check, callback) => {
  const data = validateCheckFields(check,
    (normalizedData) => normalizedData.protocol
      || normalizedData.url
      || normalizedData.method
